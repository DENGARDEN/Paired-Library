...
def do(src_file_name, dest_file_name):
	...
	try:
		...
		for barcode in barcode_data:
			...
			workbook = xlsxwriter.Workbook("results/sequence_judgment.xlsx")
			worksheet = workbook.add_worksheet("TF table")
			...
			for line in data:
				...
				if barcode.lower() in line.lower():
					# 존재한다면 대상 시퀸스는 이제 필요없으므로 추후 메모리에서 제거하기 위해 따로 보관한다. TODO 중복 시퀀스 제거로 인해 차이 ?
					used_data.append(line)

					# debug
					flag = True
					
				worksheet.write(line_cnt, 2, flag)
				line_cnt += 1 
			# debug
			workbook.close()
	...
	try:
		...
		# logging results as spreadsheet
		# 각 개별 결과 파일을 열어서
		# export as excel sheet (xlsxwriter package)
		...
		for line in open(src_file_name, "r"):
			...
			file_name = barcode_set[0].strip()
			# 라인 수를 센다음에
			count = count_line_in_file(
				os.path.join(BASE_DIR, result_folder_name, "{}.txt".format(file_name))
			)
			# 총 결과 파일에 파일명 : 라인수 형식으로 쓴다.
			worksheet.write(current_cnt, 0, file_name)
			worksheet.write(current_cnt, 1, ":")
			worksheet.write(current_cnt, 2, count)
		...
		print(e)
		print("Extraction has been done. But Making a result-info.txt is failed.")
		raise
...
print(f"Debug: input_file_path : {FASTA_file_path}")
